<?php 
	...
	$nom_fichier = "modif_cecodi.php";

	// Suppression des codes existants 
	$firstTime = true;
	...
	if ($actionCecodi == 1) {
		...
		for ($i=1;$i<6;$i++){
			...
			if ($test->Count == 0) {
				...
				// verification d'unicitŽ
				if ($firstTime) {
					$sql = "DELETE FROM cecodis2 WHERE cecodi = '$formCecodi'";
				...
				//$_SESSION['information']="Op&eacute;ration r&eacute;ussie - Le m&eacute;decin ".html_entity_decode(htmlentities(stripcslashes($formNom),ENT_QUOTES))." ".html_entity_decode(htmlentities(stripcslashes($formPrenom),ENT_QUOTES))." a &eacute;t&eacute; correctement ajout&eacute; &agrave; la base de donn&eacute;e";
...
<body id='body'>
   ...
   <?php
			...
			<div id="tab_panel">
				...
				<div class="ViewPane">
					...
					<div class="navigation-hight">
						...
						<div id='formhelp'>
							Ce formulaire permet de modifier une consultation ou un nouvel acte technique.<br />
						</div>
						<div id='formmodif'>
							<?php echo $info; ?>
						</div>
	...
	<script type='text/javascript'>
		<?php 	
			if (isset($_GET['cecodi'])) {
				$cecodi = $_GET['cecodi'];
				connexion_DB('poly');
				$sql = "SELECT id FROM cecodis2 WHERE cecodi ='$cecodi' order by 1";
				$result = requete_SQL($sql);
				$compt = 1;
				$type = 'base';
				while($data = mysql_fetch_assoc($result)) 	{
					$id = $data['id'];
					echo "loadCecodi($compt,'$type','$id');";
					$compt ++;
					$type = 'variant';
				}
				deconnexion_DB();
