<?php 
	...
	if(mysql_num_rows($result)>=1) {
		...
		$datetools = new dateTools($data['patient_date_naissance_standart'],date("Ymd"));
...
if($Handle == false){ } else{
		...
		fwrite($Handle, $DataFile); 
/*		$DataFile = "(0008,0060)=CT\n"; 
		fwrite($Handle, $DataFile); 
		$DataFile = "(0008,0080)=C.I.M.F.I. sprl\n"; 
		fwrite($Handle, $DataFile); 
		...
		fwrite($Handle, $DataFile); 
		$DataFile = "(0040,0002)=".date("Ymd")."\n"; 
		fwrite($Handle, $DataFile); 
		$DataFile = "(0040,0002)=".date("His")."\n"; 
		fwrite($Handle, $DataFile); 
		$DataFile = "(0040,0007)=".$dataCodeActe."-".$dataDescActe."\n"; 
		fwrite($Handle, $DataFile); 
		$DataFile = "(0040,0010)=CT\n"; 
		fwrite($Handle, $DataFile);
*/
