<?php 
	...
	if ($MutuelleCode != 'all') {
		...
		$sql= "select distinct ta.mutuelle_code as mutuelle_code, mu.nom as mutuelle_nom from tarifications ta, mutuelles mu where ta.etat = 'close' AND ta.mutuelle_code = mu.code".$sqlperiode." and ta.mutuelle_code='".$MutuelleCode."'";
	} else {
		// pour chaque mutuelle
		$sql= "select distinct ta.mutuelle_code as mutuelle_code, mu.nom as mutuelle_nom from tarifications ta, mutuelles mu where ta.etat = 'close' AND ta.mutuelle_code = mu.code".$sqlperiode." order by mu.code";
	}
	...
	while($data = mysql_fetch_assoc($result)) 	{
		...
		// print header
		$pdf->Cell(23,6,'ID',1,0,'L',1);
		$pdf->Cell(18,6,'Date',1,0,'L',1);
		$pdf->Cell(10,6,'Mut',1,0,'L',1);
		$pdf->Cell(55,6,'Patient',1,0,'L',1);
		$pdf->Cell(40,6,'Matricule',1,0,'L',1);
		$pdf->Cell(40,6,'Titulaire',1,0,'L',1);
		$pdf->Cell(19,6,'CT1/CT2',1,0,'L',1);
		$pdf->Cell(45,6,'Médecin',1,0,'L',1);
		...
		while($data = mysql_fetch_assoc($resulttarif)) 	{
			...
			$pdf->Cell(23,6,$id,0,0,'L',1);
			$pdf->Cell(18,6,$date,0,0,'L',1);
			$pdf->Cell(10,6,$mut,0,0,'L',1);
			$pdf->Cell(55,6,$patient,0,0,'L',1);
			$pdf->Cell(40,6,$patient_matricule,0,0,'L',1);
			$pdf->Cell(40,6,$titulaire,0,0,'L',1);
			$pdf->Cell(19,6,$patient_cts,0,0,'L',1);
			$pdf->Cell(45,6,$medecin,0,0,'L',1);
			$pdf->Cell(18,6,$cecodi,0,0,'L',1);
