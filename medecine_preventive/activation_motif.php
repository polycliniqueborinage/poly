<?php 
	...
	if($formModifierListe == 1){
		...
		while($liste = mysql_fetch_assoc($req)) 	{
			...
			$formCheckboxDel = isset($_POST['del_'.$liste['id_motif']]) ? $_POST['del_'.$liste['id_motif']] : '';

			if($formCheckboxDel == 'on'){
				$sql = "DELETE FROM `mp_activation_motifs`
						WHERE `id_motif` = '".$liste['id_motif']."'";
				$q = requete_SQL ($sql);

				$sql = "DELETE FROM `medecine_preventive`
						WHERE `motif_ID` = '".$liste['id_motif']."'";
				$q = requete_SQL ($sql);
						...
						<fieldset class=''>
							<legend>Liste des motifs de m&eacute;decine pr&eacute;ventive</legend>
								...
								if(mysql_num_rows($result)!=0) {
									...
									echo "<a target='_BLANK' onclick=openMotifInfo('".$motif['motif_ID']."')><img src='../images/icon_clipboard.gif' alt='D&eacute;tails' title='D&eacute;tails' border='0' width=15/></a>";
									echo "</td>";
									...
									echo"<td align='center'>";
									echo"<input type=checkbox id='del_".$motif['motif_ID']."' name='del_".$motif['motif_ID']."'>";
									echo"</td>";
