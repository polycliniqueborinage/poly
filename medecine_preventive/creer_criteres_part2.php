...
$formUseTablePatients     		 = isset($_POST['patients'])     		 ? $_POST['patients']      		  : '';
$formUseTableMedecins     		 = isset($_POST['medecins'])   		     ? $_POST['medecins']    		  : '';
$formUseTableVisites       		 = isset($_POST['visites'])      		 ? $_POST['visites']      		  : '';
$formUseTableTarifications       = isset($_POST['tarifications']) 		 ? $_POST['tarifications'] 		  : '';
$formUseTableTarificationsDetail = isset($_POST['tarifications_detail']) ? $_POST['tarifications_detail'] : '';
...
<form id="form1" name="form1" action="creer_criteres_part3.php" method="post">
...
<tr><td><table id="0">
	...
	<td><select id="table0" name="table0" onclick="javascript:searchSuggest(0)">
			<?
				if($formUseTableTarificationsDetail != '') echo"<option value=\"tarifications_detail\" selected>Tarifications d&eacute;tails</option>";
				if($formUseTableTarifications 		!= '') echo"<option value=\"tarifications\" selected>Tarifications</option>";
				if($formUseTableVisites       		!= '') echo"<option value=\"visites\" selected>Visites</option>";
    ...
    <td><input type=text id="valeur0" name="valeur0">
		<select id="table1000" name="table1000" onclick="javascript:searchSuggest(1000)" style='display:none'>
			<?
				if($formUseTableTarificationsDetail != '') echo"<option value=\"tarifications\" selected>Tarifications d&eacute;tails</option>";
				if($formUseTableTarifications 		!= '') echo"<option value=\"tarifications\" selected>Tarifications</option>";
				if($formUseTableVisites       		!= '') echo"<option value=\"visites\" selected>Visites</option>";
				if($formUseTableMedecins      		!= '') echo"<option value=\"medecins\" selected>M&eacute;decins</option>";
				if($formUseTablePatients      		!= '') echo"<option value=\"patients\" selected>Patients</option>";
...
<center>
<a onclick="checkCriteres(this.form);">
