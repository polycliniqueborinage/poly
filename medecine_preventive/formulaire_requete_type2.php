...
$formCecodi = isset($_POST['cecodi']) ? $_POST['cecodi'] : '';
$formBegda  = isset($_POST['begda'])  ? $_POST['begda']  : '0000-00-00';
$formEndda  = isset($_POST['endda'])  ? $_POST['endda']  : '0000-00-00';
...
if($numberOfLine == 0){
	...
	if($request == "")
		$request = "SELECT distinct patients.id FROM ".$Table." WHERE patients.id NOT IN ( SELECT tarifications.patient_id FROM tarifications, tarifications_detail WHERE tarifications.id = tarifications_detail.tarification_id AND tarifications.date >= '".$formBegda."' AND tarifications.date <= '".$formEndda."' AND tarifications_detail.cecodi = '".$formCecodi."')";
	else
		$request = "SELECT distinct patients.id FROM ".$Table." WHERE ".$request." AND  patients.id NOT IN ( SELECT tarifications.patient_id FROM tarifications, tarifications_detail WHERE tarifications.id = tarifications_detail.tarification_id AND tarifications.date >= '".$formBegda."' AND tarifications.date <= '".$formEndda."' AND tarifications_detail.cecodi = '".$formCecodi."')";
