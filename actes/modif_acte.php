<?php 
	...
	if (isset($_GET['id'])) {
		...
		// Recherche des infos sur cette acte interne
		$sql = "SELECT * FROM actes WHERE id='$formID'";
		...
		if(mysql_num_rows($result)==1) {
			...
			$formCecodis = $data['cecodis'];
			$formCoutTR = $data['cout_tr'];
			$formCoutTP = $data['cout_tp'];
			$formCoutVP = $data['cout_vp'];
			$formCoutSM = $data['cout_sm'];
			$formLength = $data['length'];
			$formID = $data['id'];
			
		} else {
	
			$_SESSION['information']="Op&eacute;ration manqu&eacute;e - Impossible de retrouver le code interne";
	...
	$actionModifActe = isset($_POST['actionModifActe']) ? $_POST['actionModifActe'] : '';

	if ($actionModifActe == 1) {
		...
		$formCoutSM = isset($_POST['cout_sm']) ? $_POST['cout_sm'] : '';
		$formCoutSM = $test->convert($formCoutSM);
		...
		if ($test->Count == 0) {
			...
			$sql= "UPDATE actes set code='$formCode', description='$formDescription', cecodis='$formCecodis', cout_vp='$formCoutVP', cout_tr='$formCoutTR' , cout_tp='$formCoutTP', cout_sm='$formCoutSM', length='$formLength' where id = '$formID'";
			
			$q = requete_SQL ($sql);
			
			// Valider l'ajout dans la DB
			$_SESSION['information']="<div id='dbinfo'>Op&eacute;ration r&eacute;ussie - Le code interne ".html_entity_decode(htmlentities(stripcslashes($formDescription),ENT_QUOTES))." a &eacute;t&eacute; correctement modifi&eacute; dans la base de donn&eacute;es</div>";
...
<head>
	<title>Poly - Modification d'un acte interne</title>
...
<body id='body'>
   ...
   <?php
    ...
    <div id="top">
		...
		<h1>Acte interne - Modificatin d'un acte interne</h1>
			...
			<div id="tab_panel">
				...
				<div class="ViewPane">
					...
					<div class="navigation-hight">
						...
						<div id='formhelp'>
							Ce formulaire permet de modifier un acte interne &agrave; condition que celui-ci existe dans l'application.<br />
						</div>
					...
					<div id="secondmain" class="formBox">
						...
						<form name='acteForm' id='acteForm' method='post' action='<?=$nom_fichier?>'>
							...
							<input type="hidden" name="actionModifActe" value="1">
							...
							<table border='1' cellspacing='0' cellpadding='0'>
								...
								<!-- Taux SM -->
								<tr>
									<th class='<?=$test->fieldError("cout_sm","required")?>'>Co&ucirc;t Sans Mutuelle</th>
									<td><input type='text' name='cout_sm' id='cout_sm' size='40' maxlength='10' class='txtField' title='Co&ucirc;t pour le Sans Mutuelle'  value='<?=$formCoutSM?>' onKeyUp='javascript:valeurcoutsm = checkAmount(this, valeurcoutsm, 10, 2, false);' autocomplete='off' />
									</td>
								</tr>
								<tr><td></td><td class="<?=$test->fieldError("cout_vp","hide")?>">Rentrer un co&ucirc;t pour le sans mutuelle</td></tr>
